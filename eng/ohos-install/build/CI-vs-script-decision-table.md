...
| 12 | aspnetcore tarball R2R overlay | installers ship the pack as built (R2R on; no out-of-tree rewrite) | stage3 overlays both `shared/Microsoft.NETCore.App/**` and `shared/Microsoft.AspNetCore.App/**` members of the aspnetcore tarball (`overlay-tarball.py`; the AspNetCore match was added 2026-09-14 after the in-tree A/B exposed the gap); release refresh run 34799213927 → tarball 43,869,639 B, runtime + aspnetcore assemblies R2R | **fixed** (before: the matcher only covered the runtime, so the AspNetCore assemblies stayed PureIL in the tarball while the pack was R2R; SCD was unaffected) |
