...
Usage: assemble-ilc-pack.py <ilc-published-dir> <reference-pack> <out-nupkg> [runtime-pack.nupkg] [framework-version] [--tfm netX.Y]
  ...
  framework-version  Version string for the deps.json runtimepack entry. This
                  ...
                  bootstrap SDK runtime, BOOTSTRAP_SDK_VERSION in ../versions.env)
                  - NOT the runtime pack file version: hostpolicy resolves
                  libcoreclr.so from the runtimepack entry version, and a
                  mismatch yields "Could not resolve CoreCLR path" on device.
                  Defaults to the runtime pack file version.
...
if len(args) < 3:
    raise SystemExit(
        "usage: assemble-ilc-pack.py <ilc-published-dir> <reference-pack> "
