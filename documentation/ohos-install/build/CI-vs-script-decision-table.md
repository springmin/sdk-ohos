...
| 3 | ReadyToRun | inside CoreCLR.sfxproj packaging, PGO mibc, full framework | full framework + CoreLib: OFFICIAL NuGet crossgen2 (26427) over all layout assemblies (`crossgen-framework.py`), overlaid into the runtime pack, layout and runtime tarball (`overlay-pack.py` / `overlay-tarball.py`); no PGO mibc on clean builds | intentional (fork crossgen2_inbuild unverified + hang history; no PGO data). Device SCD+R2R publish now compiles the app only: 43 s vs >15 min stall (2026-09-13) |
