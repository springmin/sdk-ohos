...
{
    /// <summary>
    /// Applies an OpenHarmony self-signature (.codesign section) to ELF64 binaries in place.
    /// OpenHarmony only executes ELF files that carry a valid .codesign section; a file that was
    ...
    internal static class ElfSelfSigner
    {
        ...
        private static byte[] StripCodesign(byte[] buf, out bool removed)
        {
            removed = false;
        ...
        private static byte[] SignElf(byte[] elf, bool force)
        {
            if (!IsElf64(elf))
            {
                throw new InvalidDataException("not ELF64");
